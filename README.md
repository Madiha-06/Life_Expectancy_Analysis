# Life Expectancy Analysis
A data analysis project exploring global life expectancy trends and the socioeconomic, health, and environmental factors that influence them.

## Overview
This project analyzes life expectancy data across countries and years to identify key drivers of longevity and build predictive models.It is developed using reusable pipelines with OOP format in python.
## Dataset
 - Source: Life Expectancy WHO (https://www.kaggle.com/datasets/kumarajarshi/life-expectancy-who)
 - Features: Country, Year, Life Expectancy, GDP, Population, Adult Mortality, Alcohol Consumption, BMI, Schooling,
 - Immunization Coverage (Hepatitis B, Polio, Diphtheria), HIV/AIDS Prevalence, Health Expenditure, etc
 - Time Range: (e.g., 2000–2015)
## Methodology
 * Data Cleaning — Handle missing values, correct data types, remove duplicates
 * Exploratory Data Analysis (EDA) — Summary statistics, distribution plots, correlation analysis
 * Visualization — Trends by region/country/year, comparison across income groups
 * Modeling — Regression analysis to identify the most influential predictors of life expectancy
 * Evaluation — Model performance metrics (R², RMSE, MAE)
## Tools & Libraries
 * Python 3.x
 * pandas, numpy — data manipulation
 * matplotlib, seaborn — visualization
 * scikit-learn — modeling
## Key Findings
 * Developed countries have greater life expectancy values
 * Schooling is highly correlated with life expectancy
 * For predicting Random Forest outperformed Linear regression
## Installation
  * Clone the repository
  * Use pycharm to run pipelines

   


   
